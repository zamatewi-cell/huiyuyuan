...
class AIService {
  ...
  Future<String> chat({
    ...
    // 预先计算系统提示词（供 DeepSeek 和 Gemini 共用）
    ...
    // 优先使用 DeepSeek
    if (!AppConfig.deepseekApiKey.contains('YOUR_') &&
        AppConfig.deepseekApiKey.isNotEmpty) {
      try {
    ...
    // DeepSeek 不可用时，回退到 Gemini
    if (!AppConfig.geminiApiKey.contains('YOUR_') &&
      ...
      try {
        ...
        return _filterSensitiveWords(result);
      } catch (_) {
        // Gemini 也失败，降级到离线
      }
    }
  ...
  Future<void> chatStream({
    ...
    if (!AppConfig.deepseekApiKey.contains('YOUR_') &&
      ...
      try {
        ...
        onDone(result);
        return; // DeepSeek 成功，直接返回
      } catch (_) {
        // DeepSeek 失败，尝试 Gemini
      }
    }

    // DeepSeek 不可用时，回退到 Gemini 流式
    if (!AppConfig.geminiApiKey.contains('YOUR_') &&
        AppConfig.geminiApiKey.isNotEmpty) {
      try {
        await _chatStreamWithGemini(
          userMessage: userMessage,
          history: history,
          systemPrompt: finalSystemPrompt,
          language: language,
  ...
  /// 将 DeepSeek/OpenAI 格式的历史记录转换为 Gemini contents 格式
  List<Map<String, dynamic>> _toGeminiContents(
    ...
    if (history != null) {
      for (final msg in history) {
        final role = msg['role'];
        if (role == 'system') continue; // system 单独处理
  ...
  Future<String> _chatWithGemini({
    ...
    final contents = _toGeminiContents(history, userMessage);
    final response = await _gemiDio.post(
    ...
    throw Exception('Gemini API returned no content');
  }

  /// Gemini 流式对话（SSE）
  Future<void> _chatStreamWithGemini({
    required String userMessage,
    List<Map<String, String>>? history,
    required String systemPrompt,
    String language = 'zh_CN',
    ...
    final contents = _toGeminiContents(history, userMessage);
    final response = await _gemiDio.post(
      '/models/${AppConfig.geminiModel}:streamGenerateContent',
      queryParameters: {'key': AppConfig.geminiApiKey, 'alt': 'sse'},
      data: {
        'contents': contents,
        'system_instruction': {
          'parts': [
    ...
    onDone(_filterSensitiveWords(fullResponseBuffer.toString()));
  }
