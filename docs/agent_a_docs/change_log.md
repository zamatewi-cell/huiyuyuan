# Agent A — Backend Architect Change Log

> Chronological record of all backend modifications, newest first.

---

## [2026-02-27] Session 3 — Auth Enhancement + Shops & Notifications DB Migration
...
- **NEW**: `_db_find_user(db, phone, username, operator_num, user_type)` — Generic DB user lookup helper with flexible filter params
- **NEW**: `_login_success(user_id, user_data)` — DRY helper for token generation + response assembly
...
- **NEW**: `_row_to_shop(m)` helper for DB row → Shop schema
...
- **NEW**: `POST /{id}/read` — Mark single notification as read
- **NEW**: `POST /read-all` — Mark all notifications as read for a user

#### `routers/ws.py` (139 lines, enhanced)
- **NEW**: `persist_notification(user_id, title, body, ntype, ref_id)` — Best-effort DB persistence for every WS push notification, uses own Session (non-blocking)
...
## [2026-02-27] Session 2 — PostgreSQL Data Persistence (A2)
...
#### `init_db.sql` — v4.0 Migrations Added
...
All rewrites follow the same architecture: `Depends(get_db)` → SQL via `text()` → `_row_to_model()` helper → memory fallback.
...
- **Helper**: `_row_to_product(m)` — handles JSONB images parsing, all column-to-field mapping
...
- **GET /profile**: DB `SELECT` from users, maps `avatar_url` → `avatar`, `operator_num` → `operator_number`
...
- **Helper**: `_row_to_address(m)` for DB row → Address schema conversion

#### `routers/orders.py` (701 lines) — Most Complex
- **Helpers**: `_row_to_order(m, items)`, `_ts(val)`, `_fetch_order_items(db, oid)`, `_fetch_order_with_items(db, oid)`, `_ws_notify(uid, payload)`
- **Column mapping**: DB `tracking_no` → Pydantic `tracking_number`, DB `address_snap` → Pydantic `address`
...
### Encoding Fix
- All 7 newly created router files required GBK → UTF-8 conversion due to Windows `create_file` encoding behavior
- Applied batch conversion script; verified all 14 router `.py` files as valid UTF-8

### Verification Results
- ? `from main import app` — 55 routes registered successfully
...
## [2026-02-27] Session 1 — Modularization + Security + WebSocket (A1, A3, A4)
...
- **Integration**: `_ws_notify()` helper in orders.py and admin.py for real-time push:
  - `order_created` — when buyer creates an order
  - `payment_success` — when payment confirmed
  - `order_shipped` — when admin ships an order
  - `order_cancelled` / `order_refund` — status change notifications
