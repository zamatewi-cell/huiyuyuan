...
4. 同步后端源码到 `/srv/huiyuanyuan/backend`
5. 在服务器执行 `pip install -r requirements.txt`
6. 在服务器执行 `alembic upgrade head`
7. 重启 `huiyuanyuan-backend`
8. 上传 `nginx_current.conf` 到 `/etc/nginx/conf.d/huiyuanyuan.conf`
9. `nginx -t` 成功后 reload Nginx
10. 上传前端构建产物到 `/var/www/huiyuanyuan/`
...
| `/srv/huiyuanyuan/.env` | 生产环境变量 |
| `/srv/huiyuanyuan/backend/uploads/` | 后端上传文件目录 |
...
# Nginx 日志
tail -n 50 /var/log/nginx/huiyuanyuan_access.log
tail -n 50 /var/log/nginx/huiyuanyuan_error.log
```
...
| 502 Bad Gateway | 检查 `huiyuanyuan-backend` 是否运行、Nginx upstream 是否指向 `127.0.0.1:8000` |
| HTTPS 可访问但接口报跨域 | 检查 `/srv/huiyuanyuan/.env` 中 `ALLOWED_ORIGINS` |
