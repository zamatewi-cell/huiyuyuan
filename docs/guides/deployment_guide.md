...
阿里云 ECS (47.112.98.191)
 ├─ /srv/huiyuanyuan/backend/        ← FastAPI 后端源码
 ├─ /srv/huiyuanyuan/.env            ← 生产环境变量
 ├─ /var/www/huiyuanyuan/            ← Flutter Web 静态文件
 ├─ /etc/nginx/conf.d/huiyuanyuan.conf
...
3. 服务器上已存在 Python 虚拟环境和 `huiyuanyuan-backend` 服务
...
1. 同步后端源码到 `/srv/huiyuanyuan/backend`
2. 在服务器执行 `pip install -r requirements.txt`
3. 在服务器执行 `alembic upgrade head`
4. 重启 `huiyuanyuan-backend`
5. 下发 `nginx_current.conf` 到 `/etc/nginx/conf.d/huiyuanyuan.conf`
6. `nginx -t` 成功后 reload Nginx
7. 上传 Flutter Web 构建产物到 `/var/www/huiyuanyuan/`
...
- 生产后端目录固定为 `/srv/huiyuanyuan/backend`
- 生产服务名固定为 `huiyuanyuan-backend`
...
| 路径 | 说明 |
|---|---|
| `/srv/huiyuanyuan/backend/` | 后端源码目录 |
| `/srv/huiyuanyuan/backend/venv/` | Python 虚拟环境 |
| `/srv/huiyuanyuan/.env` | 生产环境变量 |
| `/srv/huiyuanyuan/backend/uploads/` | 后端上传目录 |
| `/var/www/huiyuanyuan/` | Flutter Web 静态目录 |
| `/etc/nginx/conf.d/huiyuanyuan.conf` | 当前线上 Nginx 配置 |
| `/etc/systemd/system/huiyuanyuan-backend.service` | systemd 服务文件 |
| `/opt/huiyuanyuan/snapshots/` | 后端快照回滚目录 |
...
# 手动重启后端
systemctl restart huiyuanyuan-backend
...
| 后端启动失败 | `journalctl -u huiyuanyuan-backend -n 100 --no-pager` |
| Alembic 失败 | `cd /srv/huiyuanyuan/backend && source venv/bin/activate && alembic current` |
| Nginx 配置错误 | `nginx -t` 查看具体报错 |
| 前端页面空白 | 检查 `/var/www/huiyuanyuan/index.html` 是否存在 |
| 502 Bad Gateway | 确认 `huiyuanyuan-backend` 正在运行，且 upstream 指向 `127.0.0.1:8000` |
| HTTPS 可访问但接口跨域失败 | 检查 `/srv/huiyuanyuan/.env` 中的 `ALLOWED_ORIGINS` |
...
- [ ] `huiyuanyuan-backend` 处于 `active` 状态
