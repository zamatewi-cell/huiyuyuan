...
**工作范围**: 服务器 `47.98.188.141`、`scripts/`、`backend/*.conf`
**不碰**: 业务代码
...
| # | 问题 | 修复方案 |
|---|------|----------|
| 1 | CORS `allow_origins=["*"]` | 改为 `["http://47.98.188.141", "http://localhost:*"]` |
...
certbot --nginx -d huiyuanyuan.com -d www.huiyuanyuan.com
# 自动续期: systemctl enable certbot.timer
```

若无域名，暂时为 IP 配置自签证书或保持 HTTP。
...
server {
    ...
    server_name huiyuanyuan.com;
    
    ssl_certificate /etc/letsencrypt/live/huiyuanyuan.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/huiyuanyuan.com/privkey.pem;
