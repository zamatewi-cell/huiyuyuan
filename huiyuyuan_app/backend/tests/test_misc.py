...
@pytest.mark.asyncio
async def test_create_review_duplicate_order_rejected(
    client: AsyncClient,
    customer_auth: str,
):
    payload = {
    ...
    first = await client.post("/api/reviews", json=payload, params=auth)
    assert first.status_code == 200

    second = await client.post("/api/reviews", json=payload, params=auth)
...
@pytest.mark.asyncio
async def test_change_password(client: AsyncClient, customer_auth: str):
    auth = {"authorization": customer_auth}
...
@pytest.mark.asyncio
async def test_deactivate_customer_account(client: AsyncClient, customer_auth: str):
    auth = {"authorization": customer_auth}
    ...
    deactivate_resp = await client.post(
        ...
        json={"current_password": "Test12345"},
        params=auth,
    )
    assert deactivate_resp.status_code == 200
    assert deactivate_resp.json()["success"] is True

    profile_after = await client.get("/api/users/profile", params=auth)
    ...
    assert sms_after.status_code == 404
