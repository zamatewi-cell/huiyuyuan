...
async def _send_sms_code(client: AsyncClient, phone: str, action: str) -> str:
    resp = await client.post("/api/auth/send-sms", json={
        "phone": phone,
        "action": action,
    })
...
@pytest.mark.asyncio
async def test_customer_register_success(client: AsyncClient):
    phone = "13800009876"
    code = await _send_sms_code(client, phone, "register")
...
@pytest.mark.asyncio
async def test_customer_password_login_success(client: AsyncClient):
    phone = "13800001235"
    code = await _send_sms_code(client, phone, "register")
    ...
    assert resp.status_code == 200
    assert resp.json()["user"]["user_type"] == "customer"
...
async def test_customer_sms_login_success(client: AsyncClient):
    ...
    login_code = await _send_sms_code(client, phone, "login")
    resp = await client.post("/api/auth/verify-sms", json={
        "phone": phone,
        "code": login_code,
        "action": "login",
    })
...
@pytest.mark.asyncio
async def test_customer_sms_login_requires_existing_account(client: AsyncClient):
    phone = "13800001237"
...
@pytest.mark.asyncio
async def test_customer_register_rejects_weak_password(client: AsyncClient):
    phone = "13800001238"
    code = await _send_sms_code(client, phone, "register")
...
@pytest.mark.asyncio
async def test_customer_register_rejects_duplicate_phone(client: AsyncClient):
    phone = "13800001239"
    code = await _send_sms_code(client, phone, "register")

    first_resp = await client.post("/api/auth/register", json={
        "phone": phone,
        "code": code,
        "password": "Secure123",
        "confirm_password": "Secure123",
    ...
    second_send_resp = await client.post("/api/auth/send-sms", json={
        "phone": phone,
        "action": "register",
    })
    assert second_send_resp.status_code == 409


@pytest.mark.asyncio
async def test_customer_register_requires_terms_acceptance(client: AsyncClient):
    phone = "13800001240"
    code = await _send_sms_code(client, phone, "register")
...
@pytest.mark.asyncio
async def test_customer_reset_password_success(client: AsyncClient):
    phone = "13800001241"
