...
# Usage: bash /srv/huiyuyuan/backend/deploy_current_server.sh
# ============================================================

set -euo pipefail

APP_DIR="/srv/huiyuyuan/backend"
...
cp -a \
  ...
  deploy_current_server.sh huiyuyuan-backend.service nginx_current.conf nginx_proxy_params.conf \
...
if [ -f "${APP_DIR}/huiyuyuan-backend.service" ]; then
  cp "${APP_DIR}/huiyuyuan-backend.service" "${SYSTEMD_UNIT}"
